...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 90634
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 234980
            then (select avg(ss_ext_tax)
                  from store_sales
...
select top 100 i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (10,8,4,10,6,12) and
       d_year = 2001 and
...
select top 100 *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (DATEADD(day,-30,'2000-04-09'))
                    and (DATEADD(day,30,'2000-04-09'))
     ...
     (select avg(ss_list_price) B2_LP
      ...
      where ss_quantity between 6 and 10
        and (ss_list_price between 89 and 89+10
          or ss_coupon_amt between 6791 and 6791+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 119 and 119+10
          or ss_coupon_amt between 3787 and 3787+1000
...
select top 100 i_item_id
 ...
 and d_date between cast('2001-02-17' as date) and (DATEADD(day,60,'2001-02-17'))
 and i_manufact_id in (734,834,896,760)
...
select top 100 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (DATEADD(day,-30,'1999-04-23'))
                and (DATEADD(day,30,'1999-04-23'))
 group by
    w_state,i_item_id
...
select top 100 c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Oak Grove','Fairview','Midway','Midway','Midway') 
...
select top 100 i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2000-01-30' as date) and (DATEADD(day,60,'2000-01-30'))
 and i_manufact_id in (592,514,414,828)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (2001) and
        ((i_category in ('Sports','Men','Men') and
          i_class in ('archery','shirts','accessories')
         )
      or (i_category in ('Sports','Jewelry','Electronics') and
          i_class in ('pools','estate','memory') 
        ))
